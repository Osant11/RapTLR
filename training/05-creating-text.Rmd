...
- **`fct_var_text_group()`** — describe the distribution of a categorical variable per-group breakdown as a formatted list
...
## fct_var_text_group() {#var-text-group}

### Purpose

`fct_var_text_group()` extends `fct_var_text()` by adding a per-group breakdown. For each category of the variable of interest it reports the **overall percentage** across all subjects and the **within-group percentage** for each level of a grouping variable (e.g. treatment arm).
